...
export default function GameForm({
  ...
  const handleTagToggle = (tagId: string) => {
    ...
    console.log('Current additionalTags:', formData.additionalTags);
    
    setFormData((prev) => {
      const newTags = prev.additionalTags.includes(tagId)
        ? prev.additionalTags.filter((id) => id !== tagId)
        : [...prev.additionalTags, tagId];
      
      console.log('New additionalTags:', newTags);
      return {
        ...prev,
                ...
                {availableTags.map((tag) => (
                  <div key={tag.id} className="flex items-center">
                    <input
                      ...
                      onChange={(e) => {
                        e.stopPropagation();
                        console.log('Individual checkbox clicked for tag:', tag.id);
                        handleTagToggle(tag.id);
                      }}
                      className="mr-2 rounded"
                    />
                    <label htmlFor={`tag-${tag.id}`} className="text-sm text-gray-700 cursor-pointer">
                      {tag.name}
                    </label>
                  </div>
                ))}
